...
namespace
{
	...
	// later attempt with ERROR_SHARING_VIOLATION -- reproduced directly: writes
	// failed while SkyrimVR.exe was still running, succeeded once it exited.
	// The retry only helps a transient locker (AV/indexer), not a persistent one.
...
bool UnifiedWater::LoadOrderChanged()
{
	...
	// handles). Our plugin's own subfolder isn't -- SettingsUser.json writes there every
	// session without issue -- so the hash lives there instead of directly under Data/.
